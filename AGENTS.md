...
- The canonical public host is `https://www.5rps.jp`. The apex domain redirects
  to `www`; all canonical URLs, feeds, social metadata, and structured data
...
- RSS generation: `scripts/rss.mjs`, run after the production build.
...
- Preserve the site-level Organization, WebSite, and Movie graph and the
  BlogPosting data generated by Contentlayer. Structured data must describe
  visible, accurate site content.
- Keep draft posts out of public lists, pagination, static parameters, search,
  RSS, tag feeds, and the sitemap.
- Use source publication or modification dates for news sitemap entries. Do
  not stamp evergreen routes with the build date.
- `pnpm build` refreshes tracked generated artifacts in `public/feed.xml`,
  `public/search.json`, and `public/tags/*/feed.xml`. Review and commit those
  changes when their source content or metadata changes.
...
It must also complete the RSS postbuild step.
